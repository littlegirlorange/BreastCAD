...
class EditUtil(object):
  ...
  def _createParameterNode():
    ...
    node.SetParameter("BackgroundVolume", "None")
    node.SetParameter("LabelVolume", "None")
    node.SetParameter("CompositeNode", "None")
  ...
  @staticmethod
  def getLabelVolume(layoutName='Red'):
    compNode = EditUtil.getCompositeNode(layoutName)
    if compNode:
  ...
  @staticmethod
  def getColorNode(layoutName='Red'):
    if not EditUtil.isEraseEffectEnabled():
      EditUtil.backupLabel()
    labelNode = EditUtil.getLabelVolume(layoutName)
    if labelNode:
...
class UndoRedo(object):
  ...
  def undo(self):
    ...
    labelNode = self.undoList[-1].getVolumeNode()
    self.redoList = self.storeVolume( self.redoList, labelNode)
  ...
  def redo(self):
    ...
    if self.redoList == []:
      return
    # store current state onto undoList
    labelNode = self.redoList[-1].getVolumeNode()
    self.undoList = self.storeVolume( self.undoList, labelNode )
    # get the checkPoint to restore and remove it from the list
